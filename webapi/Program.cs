...
try
{
    using (var scope = app.Services.CreateScope())
    {
        ...
        // После context.Database.EnsureCreated();
        var tableExists = context.Database.SqlQueryRaw<bool>(
            "SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'items')").FirstOrDefault();
        Console.WriteLine($"Table 'items' exists: {tableExists}");

        // Если таблица не существует, создаем вручную
        if (!tableExists)
        {
            Console.WriteLine("Creating items table manually...");
