        ...
        while (retries > 0)
        {
            try
            {
                ...
                if (canConnect)
                {
                    ...
                    Console.WriteLine($"Table 'items' exists: {tableExists}");

                    if (!tableExists)
                    {
                        Console.WriteLine("Creating items table with single 'name' column...");

                        // Создаем таблицу с одним полем name как первичный ключ
                        ...
                        tableExists = await context.Database.SqlQueryRaw<bool>(
                            "SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'items')").FirstOrDefaultAsync();
                        Console.WriteLine($"Table verification: {tableExists}");
                    }
            ...
            {
                retries--;
                Console.WriteLine($"Database operation failed, retries left: {retries}. Error: {ex.Message}");
                if (retries == 0) throw;
                Thread.Sleep(5000);
            }
        }
